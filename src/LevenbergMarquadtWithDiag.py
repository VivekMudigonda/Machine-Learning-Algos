...
def levenberg_marquardt_fit(file_path, num_iterations, lambda_):
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        # Apply Levenberg-Marquardt update rule with λ * diag(JᵀJ)
        lambda_term = lambda_ * np.diag(np.diag(JTJ))  # Regularization term
        delta_params = np.linalg.solve(JTJ + lambda_term, JTr)  # Solve for Δp
